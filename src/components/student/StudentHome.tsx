...
export default function StudentHome() {
  ...
  // --- Seeded option shuffle (client-side only, MUST be before any early return) ---
  ...
  useEffect(() => {
    const fetchExam = async () => {
      try {
        ...
        const savedSubmissionId = savedSessionId ? localStorage.getItem(`student_submission_id_${savedSessionId}`) : null;
        const savedStep = localStorage.getItem(`current_exam_step_${savedSessionId}`);
        ...
        if (data.success) {
          ...
          localStorage.setItem('active_exam_session_id', data.sessionId);

          const savedBiodata = localStorage.getItem(`student_biodata_${data.sessionId}`);
          if (savedBiodata) {
            try {
          ...
          if (data.submissionId) {
            ...
            const localAns = localStorage.getItem(`exam_answers_${data.sessionId}`);
  ...
  const handleNextSection = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success && data.submissionId) {
        ...
        localStorage.setItem(`student_biodata_${data.sessionId}`, JSON.stringify(biodata));
        localStorage.setItem(`student_submission_id_${data.sessionId}`, data.submissionId);
        localStorage.setItem('active_exam_session_id', data.sessionId);
                ...
                return (
                  <div key={s.step} className="flex items-center">
                    <button
                      ...
                      disabled={!sessionId || !submissionId || (s.step === 1 && !!submissionId)}
                      className={`flex items-center space-x-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ${isActive
                        ...
                        : isCompleted
                          ...
                          : 'bg-slate-50 text-slate-400 hover:text-slate-600 disabled:cursor-not-allowed'
                        }`}
                    >
                      {isCompleted ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div className="space-y-2">
                        ...
                        <Select
                          ...
                          onChange={setKelas}
                          options={[
                            { value: "Akuntansi", label: "XI Akuntansi" },
                            { value: "Animasi", label: "XI Animasi" },
                            { value: "Bisnis Ritel", label: "XI Bisnis Ritel" },
                            { value: "DKV", label: "XI DKV" },
                            { value: "Layanan Perbankan", label: "XI Layanan Perbankan" },
                            { value: "Manajemen Perkantoran", label: "XI Manajemen Perkantoran" },
                          ]}
